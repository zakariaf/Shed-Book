...
| `in_app_purchase_android` `build.gradle.kts` | https://raw.githubusercontent.com/flutter/packages/main/packages/in_app_purchase/in_app_purchase_android/android/build.gradle.kts | `com.android.billingclient:billing:8.0.0` — ~~the version whose manifest could not be read~~; read on 2026-08-01 by building it (§22 A2). Its POM is where `transport-backend-cct:3.1.8` enters the graph. |
...
| ~~Mirrored Play Billing AAR manifest, **billing 2.0.3**~~ | ~~https://github.com/dandar3/android-google-play-billing/blob/master/AndroidManifest.xml~~ | **Struck 2026-08-01. Superseded by primary evidence** — `docs/gates/manifest-merger-release-report.txt` is the merger's own record of what billing 8.0.0 contributed to a real build (§22 A2, closed). Nothing cites the mirror any more. |
| **The archived merger report** | `docs/gates/manifest-merger-release-report.txt` | G0's primary evidence, 2026-08-01. Names the contributing manifest for every element in the release `.aab`. It is also G4's artefact (13 §2.6): the only thing that answers *"which library added that?"* when G1 goes red. |
...
| ~~**A2**~~ | ~~**The Play Billing 8.0.0 AAR manifest.**~~ **CLOSED 2026-08-01 by G0** (N02-T01). The guess was **right about the AAR and wrong about the graph**: `billing-8.0.0/AndroidManifest.xml` declares `com.android.vending.BILLING` and nothing else — no `ACCESS_NETWORK_STATE` — but `com.google.android.datatransport:transport-backend-cct:3.1.8`, a **compile-scope dependency** of billing 8.0.0, declares both `ACCESS_NETWORK_STATE` and `INTERNET`, and they merge. The mirror of billing 2.0.3 is no longer cited by anything. | Done. The evidence is `docs/gates/manifest-merger-release-report.txt` (lines 374 and 512–521) and it replaces the citation entirely. | Recorded in decision-record §1 item 5 and §3.2/§3.3, and `13-build-ci-release.md` §2.2/§2.3/§3.1. `android/expected_permissions.txt` holds **eight** uncommented lines, not seven. Also corrected by the same build: `WAKE_LOCK` is contributed by nothing, and `androidx.core:core:1.18.0` contributes a permission no document had listed. |
...
| ~~**B19**~~ | ~~Whether **`tools:node="remove"` in `src/main` leaves the `src/debug` `INTERNET` intact.**~~ **CLOSED 2026-08-01: it does.** The debug merger report logs the `src/main` node itself `REJECTED` — build type outranks main — and `aapt2 dump permissions` on the debug APK lists `android.permission.INTERNET`. The **profile** variant keeps it too, which nobody had asked about and `make perf` depends on. The line was written into `src/main` for the experiment and reverted before the commit. | Done, and the same experiment proved the other half: the release `.aab` built with the line drops `INTERNET` and keeps the other six. | `13-build-ci-release.md` §2.2's G0 table, row 3. |
| ~~**B20**~~ | ~~**Which `make` target trips the `package:sqlite3` build-hook download first** on a cold cache~~ **CLOSED 2026-08-01.** `pub get` first on an empty pub cache; then **`flutter test`**, which fetches the host `libsqlite3.dylib`; then `flutter build appbundle`, which fetches three more, one per Android ABI — **different artefacts**, so warming one does not warm the other. `make gen` was not measurable at N02 and N08 re-checks it. | Done. Measured on a fresh clone with an empty `PUB_CACHE`, blocking each command by pointing the proxy environment variables at a closed port, which is the block the hook honours (`HttpClient.findProxyFromEnvironment`). | `13-build-ci-release.md` §1.3 and `README.md`, both of which now carry the table. Two side findings are in the README because they cost an evening each: the hook cache is `.dart_tool/hooks_runner/shared/`, which `flutter clean` deletes; and `flutter test`'s implicit `pub get` contacts pub.dev for security advisories on a schedule of its own. |
...
| ~~**D8**~~ | ~~**G0 has not been run at all.**~~ **CLOSED 2026-08-01** (N02-T01). The effective **`minSdk` is 24**, read from `<uses-sdk android:minSdkVersion="24" android:targetSdkVersion="36"/>` in the merged manifest — it agrees with `13` §3.1's expectation, which is the outcome that needed proving rather than the one that could be assumed. **`flutter_image_compress_common` contributes `<application>` attributes and no permission.** All four cells of `13` §2.2's table are filled, each with an ISO date. | Done. `test/policy/g0_recorded_test.dart` is what keeps the record filled: it fails on the word UNVERIFIED, on a missing date, and on the two documents disagreeing about the permission set. | `13-build-ci-release.md` §2.2/§3.1 · `08-platform-integration.md` §11 items 7, 13, 14 · `11-monetization-and-store.md` §3.1–§3.2 · `CODE-REVIEW-CHECKLIST.md`. The record is true of **one dependency set** — the `pubspec.lock` of 2026-08-01 — which is what G1 exists to re-check on every push. |
...
1. ~~**G0**~~ — **run 2026-08-01, N02-T01.** Closed **A2**, **B19**, **B20** and **D8**, and the offline gate is now writable rather than unwritten. It also corrected three things nobody had listed as questions: `WAKE_LOCK` is contributed by nothing, `androidx.core` contributes a permission, and the release build does not build at all without core-library desugaring.
2. **The FTS5 migration matrix with FTS5 in schema v1** — closes **D1** and **D2** while the schema still has no real rows in it. Both are irreversible-after-the-first-snapshot decisions. **This is now first.**

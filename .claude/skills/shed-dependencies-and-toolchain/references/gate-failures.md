...
| `stream.invalidate` | `ref.invalidate(` — drift already tracks tables; manual invalidation is a stale read | Delete it (#12) **unless it is one of the two architected call sites**: `minuteTickProvider` on resume (02 §9.1) and `databaseProvider` at restore step 14 (04 §7). Neither is a drift-backed read, the rule has **no** `[exempt]` line, and `CODE-REVIEW-CHECKLIST §1.5` records that as an open gate question — raise it, never delete the call. |

...
> **G1 is writable as of 2026-08-01 and is still not written, and it is the row a reviewer most wants to trust.** 13 §2.3 implements it as `tool/assert_permissions.sh` diffing `bundletool dump manifest` on the release `.aab` against `android/expected_permissions.txt`. G0 closed the sentence that blocked it — ~~*"Until this table is filled in, `android/expected_permissions.txt` does not exist and G1 cannot be written"*~~ — but **neither the expected file nor the script nor the `android` CI job exists until N31-T02/T03.** **Do not review a diff as though the permission set were being asserted on every push.** It is not, yet.
>
...
- [x] G0 has been run against a real release AAB (**2026-08-01**, N02-T01) and its result is recorded in decision-record §3.3 **and** in 13 §2.2's four-row table. `test/policy/g0_recorded_test.dart` fails if either document loses it or if the two disagree. The `tools:node="remove"` line itself is still **not committed** — it is N31-T01's, and N02-T03's second case in `g0_recorded_test.dart` is what stops it landing early.

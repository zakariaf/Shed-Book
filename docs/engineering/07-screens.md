...
1. The commit haptic — perceivable with the phone in a bag. `06-design-system.md` §10 owns the vocabulary and spells it `HapticFeedback.successNotification()`. **That member is unverified.** The Flutter API docs for `HapticFeedback` list `vibrate`, `lightImpact`, `mediumImpact`, `heavyImpact` and `selectionClick`; the `successNotification` / `warningNotification` / `errorNotification` family and its Android `HapticFeedbackConstants.CONFIRM` mapping come from raw note 05 §10 and were never checked against the SDK. Treat the vocabulary as correct and the *spelling* as unproven: if the member does not exist on Flutter 3.44.8, the commit haptic degrades to `heavyImpact()` and the design intent is unchanged. Item 7 in §22, owned by doc 06.
...
| 7 | **`HapticFeedback.successNotification()` is unverified** | Open, and deliberately not settled by `CONVENTIONS.md` (§7 item 4: it is an SDK fact, not a name). The notification-haptic family and its `HapticFeedbackConstants.CONFIRM` mapping come from raw note 05 §10 and were never checked against the Flutter SDK. Doc 06 §10 and §11 use it in four places. If it does not exist on 3.44.8, the commit haptic is `heavyImpact()` | doc 06 |

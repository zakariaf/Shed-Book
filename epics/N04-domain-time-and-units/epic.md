...
| `docs/research/00-tech-decisions.md` | §2.E (#29, #30, #46, #47, #48, #53, #55, #56, #57), §5.1, §5.2, §7.0 | the storage shapes, one clock, canonical units, `clock 1.1.2`, `glados 1.1.7`, UK/Ireland first |
...
| **`glados` sits next to the constraint that governs the whole toolchain** | It is already a dev dependency from N00-T03 (decision-record §5.2) — unverified uploader, 2.5 y stale, adjacent to the `analyzer <13` pin that `drift_dev` needs | If `flutter pub get` reddens when you first import it in N04-T06, **delete the property layer, not the pin** (`12` §10.6). The table-driven loops in `05` §5.3 are mandatory; `glados` is additive |

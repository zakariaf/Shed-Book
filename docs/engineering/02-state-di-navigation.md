...
**Never** `ref.invalidate` a **drift-backed** read provider — after a write or at any other time. drift's `watch()` already re-emits. A manual invalidate means either the write did not go through drift or the query is missing a table in `readsFrom:`. The rule is scoped to drift-backed providers because there is exactly one legitimate invalidate in the app and it is not one: `ref.invalidate(minuteTickProvider)` on resume (§9.1), which restarts a wall-clock ticker that has no database behind it. That single call site is the whole allowance; a second one is a defect.

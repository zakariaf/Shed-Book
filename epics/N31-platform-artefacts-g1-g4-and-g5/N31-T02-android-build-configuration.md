...
| 2 | `android/app/build.gradle.kts` | **Edited.** `compileSdk = 36`, `targetSdk = 36`, `minSdk = 24` as a literal, the Kotlin JVM target at 17. `compileOptions` is **already** at Java 17 with `isCoreLibraryDesugaringEnabled = true`, and the `coreLibraryDesugaring(...)` line is **already** present — both landed at N02-T01, because the release build fails at `:app:checkReleaseAarMetadata` without them and G0 could not otherwise run. Verify; do not duplicate |

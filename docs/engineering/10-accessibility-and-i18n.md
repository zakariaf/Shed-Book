...
2. **Every message carries a `description`, and the description carries the safety rationale.** When a future contributor "improves" `withdrawalSource`, the description is what tells them why they must not. `required-resource-attributes: true` makes it non-optional.

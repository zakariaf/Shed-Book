...
| `glados` | https://pub.dev/packages/glados | Property tests for pure round-trips (12 §10.6). Unverified uploader — disclosed, dev-only, deletable. **§22 D5.** |
...
| **D5** | **`glados`'s resolution against the pinned stack** when it is actually added. Verified in decision-record §5.2 as a table entry; not re-run. | `flutter pub get` at the moment it is added. | `12-testing.md` §10.6 and Open item 5. It is the one dev dependency whose transitive graph touches the `analyzer` constraint that governs the whole toolchain — and it carries an unverified uploader and 2.5 years of staleness. **If `pub get` reddens, the property layer is deleted, not the pin.** |

...
| **Under fifteen seconds from unlock to a saved lambing** | `main()` awaits nothing and the first frame is a static dark Quick Entry shell with **no data and every target on it live** — tonight's records, the five event words, the TAG cell, `INDEX` and the slab (**P16** struck the *fully interactive keypad*: the keypad is in the tag sheet, which is closed on frame 1); the 6-tap budget is asserted in `test/features/tap_budget_test.dart` | [`01-architecture.md`](01-architecture.md) §6, [`07-screens.md`](07-screens.md) §1.3 |

...
void main() {
  ...
  test('no decision-record row marked dependency-shaped is still open', () {
    ...
    expect(
      ...
      reason:
          'in-app printing (16) and the voice-note cap (18) are the two '
    ...
    expect(
      ...
      isEmpty,
      reason:
          'these dependency-shaped questions expire when pubspec.yaml '
  ...
  test('a ruled row is struck, not deleted', () {
    ...
    for (final OpenQuestion q in questions.where((OpenQuestion q) => q.isRuled)) {
      expect(q.isStruck, isTrue, reason: 'item ${q.number} is ruled but its question text is gone');
    }
  });
  ...
  test('every surviving question carries exactly one shape tag', () {
    for (final OpenQuestion q in questions) {
      expect(
        ...
        hasLength(1),
        reason:
            'item ${q.number} carries ${q.shapes}; one shape decides '
  ...
  test('§7.0 and §7.1 agree', () {
    ...
    for (final OpenQuestion q in questions.where((OpenQuestion q) => q.isRuled)) {
      expect(
        ...
        contains(q.number),
        reason:
            'item ${q.number} is struck in §7.1 with no SETTLED row '
            'in §7.0',
      );
    }
  });
    ...
    for (final String path in documents) {
      ...
      for (final String line in lines) {
        ...
        for (final MapEntry<int, List<String>> entry in references.entries) {
          final bool namesIt = entry.value.any((String r) => lower.contains(r.toLowerCase()));
          if (!namesIt) {
            continue;
          }
